...
Dados provenientes de contagem são naturalmente discretos e frequentemente apresentam assimetria e heterocedasticidade (arrumar ref). Logo, é necessário realizar a transformação de Box-Cox, para permitir a realização de análises estatísticas mais profundas
...
Após a transformação dos dados, as suposições da análise de variância foram avaliadas por meio dos testes de normalidade e homogeneidade de variâncias.
...
As eficiência dos grãos de milho selecionados, por tipo, medida pela proporção de grãos estourados, analisada através de ANOVA, não apresentou evidência significativa de diferença entre tipos. Portanto, para o consumidor, se o objetivo for obter uma maior quantidade de pipocas estouradas, entre a Comum, Premium e Super Premium, não faz diferença qual ele compre, então, pode optar pela mais barata sem perda significativa de eficiência.

Para um estudo mais robusto, mais medidas são necessárias, como o tamanho do grão estourado, gosto, crocância ou dureza, pois limitar as classificações apenas para grão estourado ou não, limita também o que pode ser descoberto.
